...
class EnglishFuzzyGenerator(BaseFuzzyGenerator):
    ...
    def generate_variants(self, term, max_variants=30, return_phonetic_variants=False):
        ...
        2. return_phonetic_variants=True（新 API）：
           返回 List[PhoneticVariant]，包含完整語音資訊
        ...
        範例：
            ...
            >>> # 新 API（PhoneticVariant 模式）
            >>> variants = generator.generate_variants("Python", return_phonetic_variants=True)
            >>> print(variants[0])
            PhoneticVariant(text='pithon', phonetic_key='ˈpɪθɑn', score=0.92, source=PHONETIC_FUZZY)
        """
    ...
    def _generate_variants_legacy(self, term: str, max_variants: int = 30) -> List[str]:
        ...
        工作流程：
        1. **IPA 維度生成（主要）**：
           - Term → IPA：將詞彙轉換為 IPA 音標
           - IPA → Fuzzy IPA：應用音素混淆規則生成變體 IPA
           - Fuzzy IPA → Spellings：反查生成真實拼寫變體

        2. **硬編碼規則（補充）**：
           - 應用 ASR 分詞模式（如 "TensorFlow" → "ten so floor"）
           - 處理縮寫變體（如 "API" → "a p i"）
           - 應用拼寫錯誤模式

        3. **基於 IPA 去重**：
           - 計算每個變體的 IPA phonetic key
           - 移除語音相同的重複變體
        ...
        try:
            ...
            # IPA → 拼寫反查（取前 3 個拼寫）
            for ipa_var in ipa_variants[:20]:  # 限制 IPA 變體數量
                spellings = self.ipa_mapper.ipa_to_spellings(ipa_var, max_results=3)
                for spelling in spellings:
                    if spelling.lower() not in all_variants:
                        all_variants[spelling.lower()] = ipa_var

        except Exception as e:
            # IPA 生成失敗時降級到規則模式
            self._logger.debug(f"IPA generation skipped for '{term}': {e}")

        # ========== 方法 2: 硬編碼規則（補充）==========
        pattern_variants = self._apply_asr_patterns(term)
        for variant in pattern_variants:
            if variant.lower() not in all_variants:
                # 嘗試計算變體的 IPA 用於去重
                try:
                    variant_ipa = self.phonetic.to_phonetic(variant)
                    all_variants[variant.lower()] = variant_ipa
                except:
                    all_variants[variant.lower()] = ""  # 無法獲取 IPA，保留拼寫

        # ========== 去重與過濾 ==========
        # 移除原詞
        term_lower = term.lower()
        all_variants.pop(term_lower, None)
        all_variants.pop('', None)

        # 基於 IPA 去重（phonetic key 相同的只保留第一個）
        unique_variants = self._deduplicate_by_ipa(all_variants)

        # 基於 IPA 距離過濾（移除語音上過於相似的）
