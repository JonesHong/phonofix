...
class TaiwaneseAccentFuzzyToolbox:
    def __init__(self):
        # 1. 基礎聲母模糊 (Initials)
        # - zh / z、ch / c、sh / s
        # - n / l 不分
        ...
        # 反向：group -> 可能的聲母
        self.group_to_initials = {}
        for init, group in self.fuzzy_initials_map.items():
            if group not in self.group_to_initials:
                self.group_to_initials[group] = []
            self.group_to_initials[group].append(init)
        # l 也可能變成 r（補回去）
        if "r_l_group" in self.group_to_initials:
            self.group_to_initials["r_l_group"].append("l")
        ...
        for term in term_list:
            ...
            if term in self.sticky_phrase_map:
                for sticky in self.sticky_phrase_map[term]:
                    if sticky not in aliases:
                        aliases.append(sticky)

            # 原詞放在第一位
            result[term] = [term] + aliases
...
if __name__ == "__main__":
    input_terms = [
        ...
        "歡迎光臨",
        "不知道",  # 測試懶音
        "這樣",  # 測試懶音 (zhe yang -> jiang)
        "新年快樂",
        "學校",
        "鞋子",
        "然後",  # 測試 r -> l/n
        "確認",  # 測試 que -> qie, ren -> len
    ]

    toolbox = TaiwaneseAccentFuzzyToolbox()
